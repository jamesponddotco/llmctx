...
doc: # Builds the manpage.
	$(SCDOC) <doc/llmctx.1.scd >llmctx.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 llmctx $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 llmctx.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out llmctx llmctx.1
